...
parser.add_argument('--num-atoms', type=int, default=21,
                    help='Number of atoms in simulation.')
parser.add_argument('--rela-num-atoms', type=int, default=96,
                    help='Number of atoms in simulation.')
parser.add_argument('--num-edges', type=int, default=41,
                    help='Number of atoms in simulation.')
...
parser.add_argument('--edge-types', type=int, default=71,
                    help='The number of edge types to infer.')
parser.add_argument('--dims', type=int, default=4396,
                    help='The number of dimensions. 320/4396')
parser.add_argument('--save-folder', type=str, default='./checkpoints/vrd',
...
parser.add_argument('--iou', type=float, default=0.5,
                    help='Use motion capture data loader.')
parser.add_argument('--dis', type=float, default=0.45,
                    help='Use motion capture data loader.')
...
parser.add_argument('--node-types', type=int, default=101,
...
rela_test_roidb = rela_graph_roidb['test']
# ------------------------------------#
...
if args.load_folder:
    load_folder = os.path.join('./checkpoints/vrd', args.encoder +'_'
...
# ================== Data Loader ================================#
train_loader, val_loader, test_loader = load_dataset(ori_vgg=args.ori_vgg, dataset='pred', level='image',
    batch_size=args.batch_size, eval_batch_size=args.batch_size,
    shuffle=args.shuffle, feat_mode=args.feat_mode)
rela_train_loader, rela_val_loader, rela_test_loader = load_dataset(ori_vgg=args.ori_vgg, dataset='rela', level='image',
...
def iter_one_epoch(roidb, data_loader, batch_size, is_rela=False, is_training=True):
    ...
    for batch_idx, (data, target, node_cls, edge_feats, rel_rec, rel_send, bbox_loc, prior) in enumerate(data_loader):
        if args.cuda:
            ...
            bbox_loc, prior = bbox_loc.cuda(), prior.cuda()
        ...
        if args.prior:
            ...
            edge_count = Variable(torch.FloatTensor(args.edge_types / (target < args.edge_types-1).data.sum()))
            loss = edge_loss * edge_count
            # criterion = nn.MultiLabelMarginLoss().cuda()
            # loss = criterion(rel_score, target.view(-1, args.edge_types))
        else:
            # -------------- cross entropy loss (ignore the non-exist edge type) ------------- #
        ...
        if args.prior:
            if is_rela:
                output = output + torch.log(-1.0*prior)
        else:
            if is_rela:
                output = torch.log(F.softmax(output, dim=-1)) + torch.log(-1.0*prior)
            else:
...
def train(epoch, best_val_accuracy):
    ...
    if args.save_folder and recall_val > best_val_accuracy:
        ...
        # model.eval()
        # rela_loss_val, rela_acc_val, rela_recall_50, rela_recall_100, rela_pred_roidb_val = \
        #             iter_one_epoch(rela_val_roidb, rela_val_loader, args.eval_batch_size, is_rela=True, is_training=False)
        # print('--------------Best model so far---------------')
        # print('Epoch: {:04d}'.format(epoch),
        #       'loss_train: {:.04f}'.format(np.mean(loss_train)),
        #       'acc_train: {:.04f}'.format(np.mean(acc_train)),
        #       'recall_train: {:.04f}'.format(recall_train),
        #       'loss_val: {:.04f}'.format(np.mean(loss_val)),
        #       'acc_val: {:.04f}'.format(np.mean(acc_val)),
        #       'recall_val: {:.04f}'.format(recall_val))
        # print('rela_loss_val: {:.04f}'.format(np.mean(rela_loss_val)),
        #       'rela_acc_val: {:.04f}'.format(np.mean(rela_acc_val)),
        #       'rela_recall_50: {:.04f}'.format(rela_recall_50),
        #       'rela_recall_100: {:.04f}'.format(rela_recall_100),
        #       'time: {:.4f}s'.format(time.time() - t))
        # print('Epoch: {:04d}'.format(epoch),
        #       'loss_train: {:.04f}'.format(np.mean(loss_train)),
        #       'acc_train: {:.04f}'.format(np.mean(acc_train)),
        ...
        #       'acc_val: {:.04f}'.format(np.mean(acc_val)),
        #       'recall_val: {:.04f}'.format(recall_val),
        #       'rela_loss_val: {:.04f}'.format(np.mean(rela_loss_val)),
        #       'rela_acc_val: {:.04f}'.format(np.mean(rela_acc_val)),
        #       'rela_recall_50: {:.04f}'.format(rela_recall_50),
        #       'rela_recall_100: {:.04f}'.format(rela_recall_100),
...
def eval(roidb, test_loader, is_rela=False):
    ...
    if is_rela:
        rela_pred_probs = np.zeros([len(test_loader.dataset), num_edges])
        rela_pred_cls = np.zeros([len(test_loader.dataset), num_edges]) + args.edge_types - 1
    ...
    for batch_idx, (data, target, node_cls, edge_feats, rel_rec, rel_send, bbox_loc, prior) in enumerate(test_loader):
        ...
        if args.prior:
            if is_rela:
                output = output + torch.log(-1.0*prior.view(-1, args.edge_types))
        else:
            if is_rela:
                output = torch.log(F.softmax(output, dim=-1))
                rela_output = output + torch.log(-1.0*prior.view(-1, args.edge_types))
            else:
                output = F.softmax(output, dim=-1)
        ...
        if is_rela:
            rela_output = rela_output.view(-1 , num_edges, args.edge_types)
            rela_pred_prob, rela_pred_cl = rela_output.max(-1)
        
        if (batch_idx+1)*batch_size > len(test_loader.dataset):
            pred_probs[batch_idx*batch_size:] = pred_prob.data.cpu().numpy()
            pred_cls[batch_idx*batch_size:] = pred_cl.data.cpu().numpy()
            if is_rela:
                rela_pred_probs[batch_idx*batch_size:] = rela_pred_prob.data.cpu().numpy()
                rela_pred_cls[batch_idx*batch_size:] = rela_pred_cl.data.cpu().numpy()
        else:
            pred_probs[batch_idx*batch_size:(batch_idx+1)*batch_size] = pred_prob.data.cpu().numpy()
            pred_cls[batch_idx*batch_size:(batch_idx+1)*batch_size] = pred_cl.data.cpu().numpy()
            if is_rela:
                rela_pred_probs[batch_idx*batch_size:(batch_idx+1)*batch_size] = rela_pred_prob.data.cpu().numpy()
                rela_pred_cls[batch_idx*batch_size:(batch_idx+1)*batch_size] = rela_pred_cl.data.cpu().numpy()
        pbar.update(batch_size)
    pbar.close()
...
def eval_topk(roidb, test_loader, is_rela=False, k=70):
    ...
    if is_rela:
        rela_pred_probs = np.zeros([len(test_loader.dataset), num_edges, k])
        rela_pred_cls = np.zeros([len(test_loader.dataset), num_edges, k]) + args.edge_types-1
    ...
    for batch_idx, (data, target, node_cls, edge_feats, rel_rec, rel_send, bbox_loc, prior) in enumerate(test_loader):
        ...
        if is_rela:
            rela_output = rela_output.view(-1 , num_edges, args.edge_types)
            rela_pred_prob, rela_pred_cl = torch.topk(rela_output, k, dim=-1, largest=True, sorted=True)
    ...
    # if args.use_cls:
    #     print('edge_acc_test: {:.04f} node_acc_test: {:.04f}'.format(np.mean(edge_acc_test), np.mean(node_acc_test)))
